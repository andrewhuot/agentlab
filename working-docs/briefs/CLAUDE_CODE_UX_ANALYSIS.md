# Claude Code UX Analysis for AgentLab CLI

Date: 2026-03-30

This brief compares Claude Code's CLI UX to AgentLab's current workspace-first CLI and recommends the highest-leverage changes for making AgentLab feel more coherent, safer, and faster to use.
...
- AgentLab analysis from `README.md`, `docs/QUICKSTART_GUIDE.md`, `runner.py`, and the `cli/` modules.

High-level conclusion:
- AgentLab is already strong on durable workspace artifacts, config versioning, and operational surfaces.
- Claude Code feels better because it is session-first, permission-aware, and discoverable inside the terminal.
- The main product move is not "copy Claude Code's commands." It is "wrap AgentLab's existing capabilities in a unified interactive shell with memory, permissions, resumability, and a clean scripting story."
...
- AgentLab today:
  Bare `agentlab` prints help and exits instead of launching a guided flow (`runner.py:958-971`). First-run success currently depends on the user knowing `agentlab init`, then `agentlab status`, then another command (`runner.py:985-1078`, `README.md:15-22`, `docs/QUICKSTART_GUIDE.md:45-79`).
- How AgentLab should adopt it:
  Make bare `agentlab` the smart launcher.
...
- AgentLab today:
  AgentLab creates `AGENTLAB.md` and `.agentlab/workspace.json` during bootstrap (`cli/bootstrap.py:228-243`, `cli/workspace.py:21-200`). This is a good foundation, but context is still mostly one file plus workspace metadata.
- How AgentLab should adopt it:
  Evolve `AGENTLAB.md` into a full context hierarchy instead of a single memory file.
- Specific implementation recommendation:
  Support:
  1. `AGENTLAB.md` as the shared project brief.
  2. `AGENTLAB.local.md` for personal, unshared overrides.
  3. `.agentlab/rules/*.md` for focused instruction packs.
  4. `.agentlab/settings.json` for CLI behavior.
  5. `.agentlab/memory/` for generated summaries and last-session notes.
  Add a loader that merges these and exposes the active set in `agentlab status`, `agentlab doctor`, and the future shell.
...
- AgentLab today:
  AgentLab is command-per-invocation. It has isolated interactive shells for `edit --interactive` and `diagnose --interactive` (`runner.py:6251-6320`, `runner.py:6514-6556`), but no unified session shell.
- How AgentLab should adopt it:
  Add a single workspace shell that routes free text and slash commands into existing workflows.
- Specific implementation recommendation:
  Create `agentlab shell` and make it the default bare-command behavior in workspaces. V1 can be thin:
...
- AgentLab today:
  AgentLab organizes commands well at the Click layer, but discoverability depends on `--help` and documentation. The command tree is large (`runner.py:1-40`, `runner.py:146-182`).
- How AgentLab should adopt it:
...
- Specific implementation recommendation:
  ...
  - `/autofix`
  - `/exit`
  Then add AgentLab-native commands:
  - `/compact` to summarize the current shell session into `.agentlab/memory/latest_session.md`
...
- Specific implementation recommendation:
  ...
  Back these with `.agentlab/settings.json` rules such as:
...
- AgentLab today:
  AgentLab persists workspace artifacts and optimizer checkpoints, but not human conversation state. `resume` means resume the optimizer loop, not resume a CLI session (`runner.py:3133-3155`, `runner.py:2503-2583`).
- How AgentLab should adopt it:
  Persist shell sessions separately from optimization state.
- Specific implementation recommendation:
  Store sessions under `.agentlab/sessions/` with:
  - `session_id`
  - title
...
- AgentLab today:
  AgentLab has good progress output in pockets. `quickstart` and `demo quickstart` use staged output and `_stream_cycle_output` (`runner.py:5482-5646`, `runner.py:5670-5799`). Continuous optimization logs to `.agentlab/experiment_log.tsv` and prints status lines (`docs/QUICKSTART_GUIDE.md:232-266`). But progress rendering is not a shared product primitive.
- How AgentLab should adopt it:
...
- AgentLab today:
  AgentLab has `doctor` (`runner.py:2905-3045`) and some good targeted messages such as missing workspace and missing provider-key guidance (`runner.py:201-206`, `cli/mode.py:120-127`), but many commands still fail as isolated messages.
- How AgentLab should adopt it:
...
- Specific implementation recommendation:
  ...
  - `Run doctor`
  - `Docs link`
  Also add `agentlab doctor --json` and emit structured recovery fields from machine-readable commands.
...
- AgentLab today:
  AgentLab has:
  - runtime config in `agentlab.yaml`
  - workspace state in `.agentlab/workspace.json`
...
- Specific implementation recommendation:
  Introduce:
  - `~/.agentlab/config.json` for user defaults
  - `.agentlab/settings.json` for project settings
  - `.agentlab/settings.local.json` for ignored local overrides
  - session overrides stored in session metadata
  Precedence:
  flags > session > local project > project > user > defaults
  Keep `agentlab.yaml` as the agent runtime spec.
...
- AgentLab today:
  AgentLab already has meaningful `--json` coverage, but it is inconsistent. `cli/stream2_helpers.py:179-185` defines a standard `{status, data, next}` envelope, while older commands like `status --json` and `eval run --json` return raw payloads (`runner.py:2819-2841`, `runner.py:1316-1326`, `docs/QUICKSTART_GUIDE.md:779-782`).
- How AgentLab should adopt it:
...
- AgentLab today:
  AgentLab's `mcp` group only installs AgentLab into external clients by writing client config files (`cli/mcp_setup.py:55-313`). That is useful, but it is not the same as AgentLab itself having native MCP-aware runtime configuration.
- How AgentLab should adopt it:
  Split "AgentLab as an MCP server" from "AgentLab using MCP servers."
- Specific implementation recommendation:
...
- AgentLab today:
  `memory show` and `memory add` operate on `AGENTLAB.md` (`runner.py:4110-4158`). This is useful, but manual and limited to four note sections.
- How AgentLab should adopt it:
  Keep `AGENTLAB.md`, but make memory a browsable system instead of a note bucket.
- Specific implementation recommendation:
  Add:
  - `memory list`
  - `memory edit`
  ...
  - `memory where`
  - automatic session summaries written to `.agentlab/memory/`
  Extend `AGENTLAB.md` to support includes or references to focused files.
...
- AgentLab today:
  AgentLab already opens the web console from quickstart/demo (`runner.py:5644-5645`, `runner.py:5798-5799`) and integrates outward via MCP, but it does not offer strong editor/open-surface shortcuts.
- How AgentLab should adopt it:
  Add explicit "take me to the artifact" actions.
- Specific implementation recommendation:
  Add:
  - `agentlab open config`
  - `agentlab open memory`
  - `agentlab open evals`
  - `agentlab open latest-review`
  - `agentlab open console`
...
- AgentLab today:
  AgentLab already computes token and cost metrics in eval output and optimize cost summaries (`runner.py:252-274`, `runner.py:1929-1939`). It also stores a budget tracker path in runtime config bootstrap (`cli/bootstrap.py:198-207`). But cost is not a first-class home-screen concept.
- How AgentLab should adopt it:
  Promote usage from a trailing metric to a visible control surface.
- Specific implementation recommendation:
  Add `agentlab usage` and surface in `status`:
...
- AgentLab today:
  AgentLab supports multi-model runtime config in `agentlab.yaml` and documents multiple providers in the README, but there is no ergonomic model command in the CLI (`README.md:587-624`, `cli/mode.py:56-74`).
- How AgentLab should adopt it:
...
- Specific implementation recommendation:
  ...
  - `agentlab model list`
  - `agentlab model show`
  - `agentlab model set proposer <model>`
  - `agentlab model set evaluator <model>`
  - `/model` in the future shell
  Prefer writing per-workspace overrides into `.agentlab/settings.json`, not mutating `agentlab.yaml` silently.
  Files: new `cli/model.py`, `cli/mode.py`, `runner.py`, `cli/workspace.py`.
- Priority:
  P1

## Section 2: AgentLab-Specific Gaps

These are needs Claude Code does not directly solve, but Claude Code's approach is still instructive.

### A. AgentLab needs a campaign shell, not just a chat shell

Claude Code optimizes around "one conversation doing work." AgentLab needs a session that is aware of:
- active config
- latest eval
- pending review card
- pending autofix
...
Claude's task-list and status-oriented shell design is the right reference. AgentLab should copy the session feel, not the coding-agent task semantics.

### B. AgentLab needs approvals tied to deployment semantics

Claude Code permissions mostly guard file edits, commands, and web actions. AgentLab's risky actions are different:
- promoting configs
- deploying canaries
- auto-applying fixes
...
Claude's permission modes should inspire the UX, but AgentLab's policy objects should be domain-specific.

### C. AgentLab needs artifact-aware memory and compaction

Claude Code can summarize conversation state. AgentLab also needs to preserve artifact state:
...
### D. AgentLab needs team-shared project state plus local operator overrides

Claude Code's config hierarchy is useful because many users work in shared repos with local preferences. AgentLab needs the same split:
...
### E. AgentLab needs long-run automation ergonomics
...
AgentLab should converge on the same "interactive + resumable + machine-readable" contract.

### F. AgentLab needs native connector/runtime discovery, not only MCP installation

Today `agentlab mcp init claude-code` is about configuring other clients (`cli/mcp_setup.py:58-97`, `cli/mcp_setup.py:274-313`). AgentLab itself still lacks a project-scoped connector runtime. Claude Code's `.mcp.json` pattern is the right model for making tool access declarative in the workspace.

## Section 3: Concrete Implementation Roadmap

| Feature | One-line description | Effort | Files to modify | Priority |
|---|---|---:|---|---|
| Smart launcher + workspace shell | Make bare `agentlab` launch onboarding or a session shell instead of printing help | L | `runner.py`, new `cli/repl.py`, new `cli/onboarding.py`, `cli/workspace.py`, `cli/status.py`, docs | P0 |
...
| Session persistence | Persist CLI sessions under `.agentlab/sessions/` and add continue/resume commands | M | new `cli/sessions.py`, new `cli/repl.py`, `runner.py`, `cli/workspace.py` | P0 |
| CLI settings hierarchy | Separate interaction settings from `agentlab.yaml` with user/project/local/session precedence | M | new `cli/settings.py`, `runner.py`, `cli/workspace.py`, docs | P0 |
...
| Context + memory v2 | Turn `AGENTLAB.md` into a layered context system with local overrides and generated session memory | M | `core/project_memory.py`, `cli/bootstrap.py`, `runner.py`, `cli/workspace.py` | P1 |
...
| MCP runtime support | Add native MCP discovery/config consumption for AgentLab itself, not just client installation helpers | L | `cli/mcp_setup.py`, new `cli/mcp_runtime.py`, `cli/workspace.py`, `runner.py` | P1 |
...
| Openers/editor integration | Add `agentlab open ...` commands for memory, config, evals, review cards, and console | S | new `cli/openers.py`, `runner.py`, `cli/workspace.py` | P2 |
...
|---|---|---:|---|---|
| Make bare `agentlab` smart | In a workspace, show `status` or launch the shell; outside a workspace, offer `init`/`quickstart` guidance instead of dumping help | S | `runner.py`, docs | P0 |
...
| Route common failures to `doctor` | Missing workspace, missing provider credentials, and config import failures should suggest `agentlab doctor` explicitly | S | `runner.py`, `cli/mode.py` | P1 |
...
AgentLab code paths referenced most heavily:
