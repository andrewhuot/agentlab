# AgentLab VNextCC — Product Brief
...
**CLI**: `agentlab init --template customer-support` → scaffolds agent config, eval suite, and config
**App**: Onboarding wizard that does the same thing with a form

### CUJ 2: Run Evaluation
**CLI**: `agentlab eval run --config configs/v003.yaml --suite evals/cases/`
...
**CLI**: `agentlab eval results --run-id abc123` (table output)
...
**CLI**: `agentlab optimize --cycles 5 --config configs/v003.yaml`
...
**CLI**: `agentlab config diff v003 v005`
...
### CUJ 7: Deploy Config
**CLI**: `agentlab deploy --config v005 --strategy canary`
...
**CLI**: `agentlab logs --limit 20 --outcome fail`
...
**CLI**: `agentlab loop --max-cycles 50 --stop-on-plateau`
...
```
agentlab init [--template NAME]        # scaffold new project
agentlab eval run [OPTIONS]            # run eval suite
agentlab eval results [--run-id ID]    # view results
agentlab eval list                     # list recent runs
agentlab optimize [--cycles N]         # run optimization
agentlab config list                   # list config versions
agentlab config diff V1 V2             # diff two configs
agentlab config show [VERSION]         # show config YAML
agentlab deploy [--strategy canary|immediate]
agentlab loop [--max-cycles N] [--stop-on-plateau]
agentlab status                        # health dashboard (CLI)
agentlab logs [--limit N] [--outcome fail|success]
agentlab server                        # start API + web console
```
...
3. **docs/concepts.md** — how AgentLab works (the loop, configs, evals, optimization)
