# Connecting AutoAgent to Agentic Coding Tools

AutoAgent ships an MCP server so coding agents can inspect agent health, run evals, inspect traces, scaffold agents, generate eval packs, explain diffs, and prepare change sets without leaving the editor.
...
- AutoAgent now supports both `stdio` and streamable HTTP transport.
...
Connecting your coding agent to AutoAgent gives it direct access to:
...
- Python 3.11+
- A checked-out AutoAgent repo
- Recommended: install the repo in editable mode so the `autoagent` command exists
...
# Repo-local fallback from the AutoAgent repo root
python3 -m mcp_server
...
If the server is healthy, both commands return a JSON-RPC response with `serverInfo.name = "autoagent"`.

## Shared AutoAgent MCP surface

Once any client is connected, it sees the same AutoAgent MCP surface:
...
|------|--------------|-------------|
| `autoagent_status` | Returns current health, scores, and failure summary. | "Run `autoagent_status` and summarize the biggest current risk." |
| `autoagent_explain` | Produces a plain-English health summary. | "Use `autoagent_explain` and tell me whether the agent is stable enough to ship." |
| `autoagent_diagnose` | Starts a diagnosis session with clustered failure analysis. | "Run `autoagent_diagnose` and show the top failure clusters." |
| `autoagent_get_failures` | Returns recent failure samples, optionally filtered by failure family. | "Call `autoagent_get_failures` for billing-related failures." |
| `autoagent_suggest_fix` | Suggests a config fix from a natural-language description. | "Use `autoagent_suggest_fix` to make the assistant more concise without weakening safety." |
| `autoagent_edit` | Applies a natural-language config edit and returns the proposed diff/result. | "Run `autoagent_edit` to tighten refund-policy routing." |
| `autoagent_eval` | Runs the eval suite and returns scores. | "Run `autoagent_eval` on `configs/v012.yaml` before I merge this." |
| `autoagent_eval_compare` | Runs evals on two configs and returns the winner. | "Compare the active config to `configs/candidate.yaml` with `autoagent_eval_compare`." |
| `autoagent_skill_gaps` | Identifies capability gaps from failures. | "Call `autoagent_skill_gaps` and tell me what skills we are missing." |
| `autoagent_skill_recommend` | Recommends skills from the registry. | "Use `autoagent_skill_recommend` for the current failure pattern." |
| `autoagent_replay` | Returns optimization history. | "Show me the last five optimization attempts with `autoagent_replay`." |
| `autoagent_diff` | Diffs two config versions. | "Explain what changed between versions 3 and 5 using `autoagent_diff`." |
...
AutoAgent also exposes five MCP prompts:
...
- AutoAgent repo checked out locally
- Either:
  - `autoagent` available on `PATH` after `pip install -e ".[dev]"`, or
...
```bash
claude mcp add --transport stdio autoagent -- autoagent mcp-server
```

Repo-local fallback:

```bash
claude mcp add --transport stdio autoagent -- python3 -m mcp_server
```
...
```text
Run autoagent_status and summarize the top two failure buckets.
```

### Common workflows

- Check agent health from your editor:
  - "Run `autoagent_status` and `autoagent_explain`, then tell me if the active config is safe to keep."
- Run an eval while coding:
  - "Use `autoagent_eval` on `configs/v013.yaml` and compare it to the active config."
- Deploy a config change:
  - "Use `autoagent_edit`, then `autoagent_eval`, then `open_pr`. If the PR is ready, tell me the exact `autoagent deploy` command to run."
- View traces for debugging:
  - "Use `inspect_trace` and `autoagent_get_failures` for the latest refund-related failures."
- Trigger optimization:
  - "Start with `autoagent_diagnose`, then use `autoagent_suggest_fix`, `autoagent_edit`, and `run_benchmark` to propose the next optimization."
...
- AutoAgent available either as:
  - installed CLI: `autoagent mcp-server`
...
```bash
codex mcp add autoagent -- autoagent mcp-server
```

Repo-local fallback:

```bash
codex mcp add autoagent -- python3 -m mcp_server
```
...
```bash
python3 -m mcp_server --host 127.0.0.1 --port 8765
codex mcp add autoagent-http --url http://127.0.0.1:8765/mcp
```
...
```bash
codex mcp list
codex mcp get autoagent
```

Then ask Codex:

```text
Use autoagent_status to check the current health of this agent project.
```

### Common workflows

- Check agent health from your editor:
  - "Use `autoagent_status` and `autoagent_explain`, then summarize the risk in one paragraph."
- Run an eval while coding:
  - "Use `autoagent_eval_compare` on the current candidate config and the active config."
- Deploy a config change:
  - "Use `autoagent_edit`, run `autoagent_eval`, and if the result is better, prepare a PR with `open_pr`."
- View traces for debugging:
  - "Inspect the latest failed trace and group the failures with `autoagent_diagnose`."
- Trigger optimization:
  - "Run `autoagent_diagnose`, then propose a fix with `autoagent_suggest_fix`, benchmark it, and tell me whether to promote it."
...
Because Jules does not currently expose arbitrary raw MCP config for local servers, there is no verified direct AutoAgent verification command to document here. The practical verification today is to confirm whether `Settings -> MCP` offers custom endpoint registration in your tenant. If it does not, treat Jules support for AutoAgent as unavailable today.

### Common workflows

If Google opens custom MCP endpoints, the same shared AutoAgent tool catalog in this guide is what Jules would consume. Until then, use another MCP-capable coding client for:
...
- AutoAgent available via `autoagent mcp-server` or `python3 -m mcp_server`
...
- Open Agent / Composer and confirm `autoagent` appears in the available tools.
- If you use the Cursor CLI, the docs expose MCP management under `cursor-agent mcp`.

Then ask:

```text
Run autoagent_status and tell me whether the latest eval score is improving.
```

### Common workflows

- Check agent health from your editor:
  - "Use `autoagent_status` and summarize the current failure buckets."
- Run an eval while coding:
  - "Run `autoagent_eval` against the candidate config I just edited."
- Deploy a config change:
  - "Prepare the change with `autoagent_edit`, verify it with `autoagent_eval`, then use `open_pr`."
- View traces for debugging:
  - "Use `inspect_trace` and `autoagent_get_failures` for the latest escalation bug."
- Trigger optimization:
...
3. Confirm `autoagent` is installed and enabled.
4. Ask Cascade:

```text
Use autoagent_status and summarize whether the active agent is healthy.
```

Windsurf currently enforces a 100-tool total cap in Cascade. AutoAgent contributes 22 tools by itself, so if you connect many other MCP servers you may need to disable some tools in the Windsurf MCP UI.

### Common workflows

- Check agent health from your editor:
  - "Run `autoagent_status` and `autoagent_explain`, then tell me the top operational risk."
- Run an eval while coding:
  - "Run `autoagent_eval_compare` on the active config and the candidate config I am editing."
- Deploy a config change:
  - "Use `autoagent_edit`, then `autoagent_eval`, then `open_pr`. Tell me the deploy command after the PR is ready."
- View traces for debugging:
...
If the client prefers streamable HTTP, start AutoAgent locally:
...
If `tools/list` does not return the 22-tool registry described above, the problem is in client launch/config, not in the AutoAgent MCP server.
...
- You are not in the AutoAgent repo root and the package is not installed.
...
If you front AutoAgent with an HTTPS proxy and use HTTP mode:
...
```text
autoagent://configs/<file>
```

### Tool is missing in Windsurf

Windsurf caps Cascade at 100 enabled MCP tools total. Disable unused tools from other MCP servers or toggle down the AutoAgent tools you do not need.
...
### Jules cannot add AutoAgent

That is the current product limitation, not an AutoAgent server bug. Use another client until Google exposes custom endpoint registration for your tenant.
...
```bash
nohup python3 -m mcp_server --host 127.0.0.1 --port 8765 >/tmp/autoagent-mcp.log 2>&1 &
```
...
This repo already includes a `Dockerfile`, so you can expose AutoAgent over HTTP from a container:

```bash
docker build -t autoagent-mcp .
docker run --rm -p 8765:8765 autoagent-mcp \
...
- install AutoAgent in the image
- expose `autoagent mcp-server`
