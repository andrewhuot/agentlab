...
export function AdkDeploy() {
  ...
  const [target, setTarget] = useState<'cloudrun' | 'vertexai'>('cloudrun');
  ...
  function handleDeploy() {
    ...
    deployMutation.mutate(
      { agent_path: agentPath, target, project_id: projectId, region },
      {
        onSuccess: (result) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-4 space-y-3">
        ...
        <div className="space-y-2">
          <input
            ...
            className="w-full bg-zinc-900 border border-zinc-600 rounded px-3 py-1.5 text-sm text-zinc-200 focus:border-blue-500 focus:outline-none"
          />
      ...
      {/* Deployment target */}
      <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-4 space-y-3">
        <h3 className="text-sm font-medium text-zinc-200">Deployment Target</h3>
        <div className="flex gap-3">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="radio"
              value="cloudrun"
              checked={target === 'cloudrun'}
              onChange={(e) => setTarget(e.target.value as 'cloudrun')}
              className="text-blue-600"
            />
            <Cloud className="w-4 h-4 text-zinc-400" />
            <span className="text-sm text-zinc-200">Cloud Run</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="radio"
              value="vertexai"
              checked={target === 'vertexai'}
              onChange={(e) => setTarget(e.target.value as 'vertexai')}
              className="text-blue-600"
            />
            <Brain className="w-4 h-4 text-zinc-400" />
            <span className="text-sm text-zinc-200">Vertex AI</span>
          </label>
        </div>
      ...
      {/* Preview changes */}
      <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-4 space-y-3">
        <h3 className="text-sm font-medium text-zinc-200">Preview Changes (Optional)</h3>
        <div className="grid grid-cols-2 gap-2">
          <input
            type="text"
        ...
        {showPreview && diffLoading && (
          <p className="text-xs text-zinc-400">Loading diff...</p>
        )}
        {showPreview && diffData && (
          <div className="space-y-2">
            ...
            <div className="bg-zinc-900 border border-zinc-700 rounded p-3 text-xs text-zinc-300 overflow-x-auto max-h-64 overflow-y-auto">
      ...
      <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-4 space-y-3">
        ...
        >
          ...
          {deployMutation.isPending ? 'Deploying...' : 'Deploy to ' + (target === 'cloudrun' ? 'Cloud Run' : 'Vertex AI')}
        </button>
        {deployMutation.data && (
          <div className="space-y-2">
            ...
            <div className="text-sm text-zinc-300 space-y-1">
              <p><span className="text-zinc-500">Target:</span> {deployMutation.data.target}</p>
