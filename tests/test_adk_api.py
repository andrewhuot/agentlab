...
def mock_adk_tree():
    ...
    return AdkAgentTree(root=agent)
...
def test_import_endpoint(mock_import_result):
    ...
    with patch("adk.AdkImporter") as MockImporter:
        mock_instance = Mock()
...
def test_export_endpoint(mock_export_result):
    ...
    with patch("adk.AdkExporter") as MockExporter:
        mock_instance = Mock()
...
def test_deploy_endpoint(mock_deploy_result):
    ...
    with patch("adk.AdkDeployer") as MockDeployer:
        mock_instance = Mock()
...
def test_status_endpoint(mock_adk_tree):
    ...
    with patch("adk.AdkParser") as MockParser:
        mock_instance = Mock()
        mock_instance.parse_agent_directory.return_value = mock_adk_tree
...
def test_diff_endpoint():
    ...
    with patch("adk.AdkExporter") as MockExporter,          patch("builtins.open", create=True) as mock_open:
        from io import StringIO
        mock_open.return_value.__enter__.return_value = StringIO("prompts:\n  root: Test")
        mock_instance = Mock()
        mock_instance.preview_changes.return_value = [
            {"file": "agent.py", "field": "instruction"}
        ]
        MockExporter.return_value = mock_instance
