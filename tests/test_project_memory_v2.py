...
def test_load_layered_project_context_merges_all_sources(tmp_path: Path) -> None:
    ...
    (tmp_path / "AGENTLAB.md").write_text("# Shared\nAlways keep the refund flow fast.\n", encoding="utf-8")
    (tmp_path / "AGENTLAB.local.md").write_text("# Local\nPrefer concise debug output.\n", encoding="utf-8")
    rules_dir = tmp_path / ".agentlab" / "rules"
    memory_dir = tmp_path / ".agentlab" / "memory"
    ...
    assert snapshot.shared_path.name == "AGENTLAB.md"
    assert snapshot.local_path.name == "AGENTLAB.local.md"
...
def test_write_session_summary_creates_generated_memory_file(tmp_path: Path) -> None:
    """Summaries should be stored under `.agentlab/memory/`."""
    ...
    assert written.parent == tmp_path / ".agentlab" / "memory"
...
def test_memory_where_reports_active_sources(runner: CliRunner) -> None:
    ...
    with runner.isolated_filesystem():
        ...
        Path("AGENTLAB.local.md").write_text("# Local\nPersonal notes.\n", encoding="utf-8")
        Path(".agentlab/rules").mkdir(parents=True, exist_ok=True)
        Path(".agentlab/rules/routing.md").write_text("# Routing\nPrefer billing specialist.\n", encoding="utf-8")

        result = runner.invoke(cli, ["memory", "where"])

        assert result.exit_code == 0, result.output
        assert "AGENTLAB.md" in result.output
        assert "AGENTLAB.local.md" in result.output
        assert ".agentlab/rules/routing.md" in result.output
...
def test_memory_edit_append_creates_target_file(runner: CliRunner) -> None:
    ...
    with runner.isolated_filesystem():
        ...
        assert result.exit_code == 0, result.output
        body = Path("AGENTLAB.local.md").read_text(encoding="utf-8")
...
def test_memory_summarize_session_writes_latest_session_note(runner: CliRunner) -> None:
    ...
    with runner.isolated_filesystem():
        ...
        assert result.exit_code == 0, result.output
        written_files = list((Path(".agentlab") / "memory").glob("*session*.md"))
